...
void TrainManager::writeTrain(int index, Train &train) {
    writeTrain(index, train.stationNum, train.type, train.state, train.stations, train.sum_prices, train.seats, train.arrivingTimes, train.leavingTimes);
}

void TrainManager::writeTrain(int index, int stationNum, char type, int state, sjtu::vector<int> &stations, sjtu::vector<int> &sum_prices, sjtu::vector<int> &seats, sjtu::vector<int> &arrivingTimes, sjtu::vector<int> leavingTimes) {
...
TrainManager::TrainManager() : train_filename("traindata"), station_filename("stationdata"), train_bpt("trainbasic", "trainindex"), station_bpt("stationbasic", "stationindex"), stationdate_bpt("stationdatebasic", "stationdateindex") {
...
sjtu::vector<int> TrainManager::getTrainIndex(const sjtu::string<20> &trainID) {
...
void TrainManager::addTrain(const sjtu::string<20> &trainID, int stationNum, int seatNum, sjtu::vector<sjtu::string<30>> &stations, sjtu::vector<int> &prices, int startTime, sjtu::vector<int> &travelTimes, sjtu::vector<int> &stopoverTimes, sjtu::pair<int, int> saleDate, char type) {
    ...
    int sum_time = saleDate.first * 86400 + startTime;
    int sum_price = 0;
    ...
    leavingtimes.push_back(0);
    sum_prices.push_back(sum_price + prices[stationNum - 2]);
    writeTrain(train_count, stationNum, type, 0, station_index, sum_prices, seats, arrivingtimes, leavingtimes);
...
void TrainManager::deleteTrain(sjtu::vector<int> &indexs) {
    ...
    for (int index : indexs) {
        train_file.seekp(sizeofTrain * index + sizeofint + sizeofchar);
        train_file.write(reinterpret_cast<char*>(&state), sizeofint);
    }
}
